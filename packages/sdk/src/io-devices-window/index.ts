import type { IoDevices } from '../io-devices'

export class IoDeviceWindow {
  ...
  _eventListenerKeydown(event: KeyboardEvent): void {
    ...
    if (!this.popupKeyCodes.includes(keyCode))
      this.popupKeyCodes.push(keyCode)
  }

  _eventListenerKeyup(event: KeyboardEvent): void {
    event.preventDefault()
    const keyCode = event.keyCode
    const keyCodeIndex = this.popupKeyCodes.indexOf(keyCode)
    if (keyCodeIndex > -1)
  ...
  _getPositionFromMouseEvent(
    ...
    // Find where X and Y would be according to the scale
