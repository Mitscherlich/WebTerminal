...
export class Shell {
  ...
  handleCursorMove = (dir: number) => {
    if (dir > 0) {
      const num = Math.min(
        dir,
        this.tty.getInput().length - this.tty.getCursor(),
      )
  ...
  handleTermData = (data: string) => {
    ...
    if (this.tty.getFirstInit() && this._activePrompt) {
      const line = this.tty
        .getBuffer()
        .getLine(
          this.tty.getBuffer().cursorY + this.tty.getBuffer().baseY,
        )
      const promptRead = (line as IBufferLine).translateToString(
        false,
        0,
    ...
    if (ord === 0x1B) {
      switch (data.substr(1)) {
        ...
        case '\x7F': // CTRL + BACKSPACE
          ...
          if (ofs) {
            this.tty.setInput(
              this.tty.getInput().substr(0, ofs)
                + this.tty.getInput().substr(this.tty.getCursor()),
            )
            this.tty.setCursor(ofs)
          }
      ...
      switch (data) {
        ...
        case '\t': // TAB
          if (this._autocompleteHandlers.length > 0) {
            const inputFragment = this.tty
              .getInput()
              .substr(0, this.tty.getCursor())
