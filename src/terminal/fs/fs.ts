...
import { Volume, filenameToSteps } from 'memfs/lib/volume'
import type { Link } from 'memfs/lib/node'
import { pathToFilename } from 'memfs/lib/node/util'
...
export class FsContainer {
  ...
  private _toJSON(link: Link, json: any = {}, path?: string): DirectoryJSON {
    ...
    for (const [name] of link.children) {
      if (['.', '..'].includes(name))
        continue
